...
contract('Staking', ([owner, alice, bob, carl]) => {
    before(async () => {
        ...
        this.boost = await ERC20.new("Boost Token", "BST", 0, 0);

        const currentBlock = await web3.eth.getBlockNumber();
        this.startBlock = currentBlock + 100;

        this.staking = await Staking.new(this.erc20.address, 100, this.startBlock);
        this.staking.add(15, this.lp.address, false);
    ...
    before(async () => {
        await Promise.all([
            this.boost.mint(alice, 1000),
            this.boost.mint(carl, 800),
        ]);

        const [balanceAlice, balanceBob, balanceCarl] = await Promise.all([
            this.boost.balanceOf(alice),
            this.boost.balanceOf(bob),
            this.boost.balanceOf(carl),
        ]);
    ...
    describe('when created', () => {
        ...
        it('holds 10,000 MOCK', async () => {
            const balance = await this.erc20.balanceOf(this.staking.address);
    ...
    describe('before the start block', () => {
        before(async () => {
            await Promise.all([
                this.lp.approve(this.staking.address, 1500, { from: alice }),
                this.lp.approve(this.staking.address, 500, { from: bob })
            ]);
        ...
        it('allows participants to join', async () => {
            const balanceStaking = await this.lp.balanceOf(this.staking.address);
    ...
    describe('after 10 blocks of stakinging', () => {
        before(async () => {
    ...
    describe('with a 3th participant after 30 blocks', () => {
        before(async () => {
            ...
            await this.lp.approve(this.staking.address, 2000, { from: carl });
    ...
    describe('after 50 blocks of stakinging', () => {
        before(async () => {
    ...
    describe('with a participant partially withdrawing after 80 blocks', () => {
        before(async () => {
            ...
            await this.staking.withdraw(0, 1500, {from: carl});
        });
    ...
    describe('after 140 blocks of stakinging', () => {
        before(async () => {
    ...
    describe('with a participant partially withdrawing BST after 150 blocks', () => {
        ...
        it('gives carl 500 MOCK and 200 LP', async () => {
            ...
            const balanceLP = await this.boost.balanceOf(carl);
